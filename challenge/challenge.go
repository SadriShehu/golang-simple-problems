...
func BillFor(yearMonth string, activeSubscription *Subscription, users *[]User) float64 {
	// if we have no subscription retrun 0.0
	if activeSubscription == nil {
		return 0.0
	}

	// if we have no users return 0.0
	if len(*users) == 0 {
		return 0.0
	}

	// convert the yearMonth to time.Time
	yearMonthTime, err := time.Parse("2006-01", yearMonth)
	if err != nil {
		fmt.Printf("error during time parsing %s\n", err.Error())
		return 0.0
	}

	currentMonth := yearMonthTime.Month()
	currentYear := yearMonthTime.Year()
	lastDayOfCurrentMonth := LastDayOfMonth(yearMonthTime)
	_, _, lastDay := lastDayOfCurrentMonth.Date()

	var total float64

	for _, user := range *users {

		// check if the user is form the subscription customer
		...
		_, _, activationDay := user.ActivatedOn.Date()
		priceForDay := float64(activeSubscription.MonthlyPriceInDollars) / float64(lastDay)

		if user.DeactivatedOn.IsZero() {
			if user.ActivatedOn.Year() != currentYear {
				total += float64(activeSubscription.MonthlyPriceInDollars)
				continue
			}

			if user.ActivatedOn.Month() != currentMonth {
				total += float64(activeSubscription.MonthlyPriceInDollars)
				continue
			}

			if activationDay > 1 {
				priceForDays := priceForDay * (float64(lastDay) - float64(activationDay-1))
				total += priceForDays
				continue
			}
		}

		// check if the user was deactivated during the current year
		if user.DeactivatedOn.Year() != currentYear {
			continue
		}

		// check if the user was deactivated during the current month
		if user.DeactivatedOn.Month() != currentMonth {
			continue
		}

		_, _, deactivationDay := user.DeactivatedOn.Date()
		if user.ActivatedOn.Month() == currentMonth && activationDay > 1 {
			priceForDays := priceForDay * (float64(deactivationDay) - float64(activationDay-1))
			total += priceForDays
			continue
		}

		priceForDays := priceForDay * (float64(deactivationDay))
		total += priceForDays
		continue
	}
